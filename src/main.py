...
def main():
    ...
    # Фильтрация вакансий
    filtered_vacancies = OperationsWithVacancies("java", level, employment_type, currency, salary_from, salary_to)
    print(filtered_vacancies._filtered_vacancies())

    # Анализ зарплат
    salary_info = SalaryOfVacancies(language, level, employment_type, currency, salary_from, salary_to)
    print(salary_info._highest_pay())

    # Сохранение данных о вакансиях
    data_to_file = SaveData(language, level, employment_type, currency, salary_from, salary_to)
    print(data_to_file._save_data())

    # Получение данных из файла
    data_from_file = GetData(language, level, employment_type, currency, salary_from, salary_to)
    print(data_from_file._get_data())

    # Пользовательский ввод
    input_data = input("Введите информацию через запятую (ЯП, уровень, форма занятости, валюта, зарплата от, до): ")
    key_word, name, employment, currency, pay_from, pay_to = map(str.strip, input_data.split(","))
